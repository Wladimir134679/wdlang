...
public final class Parser {
    ...
    public static Statement parse(ScriptProgram scriptProgram, List<Token> tokens) {
    ...
    public Parser(ScriptProgram scriptProgram, List<Token> tokens) {
    ...
    private Statement statement() {
        ...
        if (match(TokenType.USE)) {
            return new UseStatement(scriptProgram, expression());
        }
        if (match(TokenType.INCLUDE)) {
            return new IncludeStatement(scriptProgram, expression());
        }
    ...
    private DestructuringAssignmentStatement destructuringAssignment() {
        ...
        return new DestructuringAssignmentStatement(scriptProgram, variables, expression());
    }
    ...
    private ForeachArrayStatement foreachArrayStatement() {
        ...
        final Statement statement = statementOrBlock();
        return new ForeachArrayStatement(scriptProgram, variable, container, statement);
    }
    ...
    private ForeachMapStatement foreachMapStatement() {
        ...
        final Statement statement = statementOrBlock();
        return new ForeachMapStatement(scriptProgram, key, value, container, statement);
    }
    ...
    private FunctionDefineStatement functionDefine() {
        ...
        return new FunctionDefineStatement(scriptProgram, name, arguments, body);
    }
    ...
    private Expression functionChain(Expression qualifiedNameExpr) {
        ...
        if (lookMatch(0, TokenType.DOT)) {
            ...
            if (lookMatch(0, TokenType.LPAREN)) {
                // next function call
                return functionChain(new ContainerAccessExpression(scriptProgram, expr, indices));
            }
            // container access
            return new ContainerAccessExpression(scriptProgram, expr, indices);
        }
        return expr;
    }
    ...
    private FunctionalExpression function(Expression qualifiedNameExpr) {
        // function(arg1, arg2, ...)
        consume(TokenType.LPAREN);
        final FunctionalExpression function = new FunctionalExpression(scriptProgram, qualifiedNameExpr);
        while (!match(TokenType.RPAREN)) {
    ...
    private MatchExpression match() {
        ...
        return new MatchExpression(scriptProgram, expression, patterns);
    }
    ...
    private Statement classDeclaration() {
        ...
        // }
        final String name = consume(TokenType.WORD).getText();
        final ClassDeclarationStatement classDeclaration = new ClassDeclarationStatement(scriptProgram, name);
        consume(TokenType.LBRACE);
        do {
            if (match(TokenType.DEF)) {
    ...
    private AssignmentExpression assignmentStrict() {
        ...
        return new AssignmentExpression(scriptProgram, op, (Accessible) targetExpr, expression);
    }
    ...
    private Expression ternary() {
        ...
        if (match(TokenType.QUESTIONCOLON)) {
            return new BinaryExpression(scriptProgram, BinaryExpression.Operator.ELVIS, result, expression());
        }
        return result;
    }
    ...
    private Expression bitwiseOr() {
        ...
        while (true) {
            if (match(TokenType.BAR)) {
                expression = new BinaryExpression(scriptProgram, BinaryExpression.Operator.OR, expression, bitwiseXor());
                continue;
            }
            break;
        }
    ...
    private Expression bitwiseXor() {
        ...
        while (true) {
            if (match(TokenType.CARET)) {
                expression = new BinaryExpression(scriptProgram, BinaryExpression.Operator.XOR, expression, bitwiseAnd());
                continue;
            }
            break;
        }
    ...
    private Expression bitwiseAnd() {
        ...
        while (true) {
            if (match(TokenType.AMP)) {
                expression = new BinaryExpression(scriptProgram, BinaryExpression.Operator.AND, expression, equality());
                continue;
            }
            break;
        }
    ...
    private Expression shift() {
        ...
        while (true) {
            if (match(TokenType.LTLT)) {
                expression = new BinaryExpression(scriptProgram, BinaryExpression.Operator.LSHIFT, expression, additive());
                continue;
            }
            if (match(TokenType.GTGT)) {
                expression = new BinaryExpression(scriptProgram, BinaryExpression.Operator.RSHIFT, expression, additive());
                continue;
            }
            if (match(TokenType.GTGTGT)) {
                expression = new BinaryExpression(scriptProgram, BinaryExpression.Operator.URSHIFT, expression, additive());
                continue;
            }
            if (match(TokenType.DOTDOT)) {
                expression = new BinaryExpression(scriptProgram, BinaryExpression.Operator.RANGE, expression, additive());
                continue;
            }
            break;
        }
    ...
    private Expression additive() {
        ...
        while (true) {
            if (match(TokenType.PLUS)) {
                result = new BinaryExpression(scriptProgram, BinaryExpression.Operator.ADD, result, multiplicative());
                continue;
            }
            if (match(TokenType.MINUS)) {
                result = new BinaryExpression(scriptProgram, BinaryExpression.Operator.SUBTRACT, result, multiplicative());
                continue;
            }
            if (match(TokenType.COLONCOLON)) {
                result = new BinaryExpression(scriptProgram, BinaryExpression.Operator.PUSH, result, multiplicative());
                continue;
            }
            if (match(TokenType.AT)) {
                result = new BinaryExpression(scriptProgram, BinaryExpression.Operator.AT, result, multiplicative());
                continue;
            }
            if (match(TokenType.CARETCARET)) {
                result = new BinaryExpression(scriptProgram, BinaryExpression.Operator.CARETCARET, result, multiplicative());
                continue;
            }
            break;
        }
    ...
    private Expression multiplicative() {
        ...
        while (true) {
            if (match(TokenType.STAR)) {
                result = new BinaryExpression(scriptProgram, BinaryExpression.Operator.MULTIPLY, result, objectCreation());
                continue;
            }
            if (match(TokenType.SLASH)) {
                result = new BinaryExpression(scriptProgram, BinaryExpression.Operator.DIVIDE, result, objectCreation());
                continue;
            }
            if (match(TokenType.PERCENT)) {
                result = new BinaryExpression(scriptProgram, BinaryExpression.Operator.REMAINDER, result, objectCreation());
                continue;
            }
            if (match(TokenType.STARSTAR)) {
                result = new BinaryExpression(scriptProgram, BinaryExpression.Operator.POWER, result, objectCreation());
                continue;
            }
            break;
        }
    ...
    private Expression objectCreation() {
       if (match(TokenType.NEW)) {
            ...
            return new ObjectCreationExpression(scriptProgram, className, args);
        }
    ...
    private Expression primary() {
        ...
        if (match(TokenType.COLONCOLON)) {
            ...
            return new FunctionReferenceExpression(scriptProgram, functionName);
        }
        if (match(TokenType.MATCH)) {
            return match();
        }
        if (match(TokenType.DEF)) {
            ...
            return new ValueExpression(new UserDefinedFunction(scriptProgram, arguments, statement));
        }
        return variable();
    }
    ...
    private Expression qualifiedName() {
        ...
        final List<Expression> indices = variableSuffix();
        if (indices == null || indices.isEmpty()) {
            return new VariableExpression(scriptProgram, current.getText());
        }
        return new ContainerAccessExpression(current.getText(), indices, scriptProgram);
    }
            ...
            if (lookMatch(0, TokenType.DOT)) {
                if (lookMatch(1, TokenType.WORD) && lookMatch(2, TokenType.LPAREN)) {
                    ...
                    return functionChain(new ContainerAccessExpression(
                            scriptProgram, strExpr, Collections.singletonList(
                ...
                return new ContainerAccessExpression(scriptProgram, strExpr, indices);
            }
            return strExpr;
        }
