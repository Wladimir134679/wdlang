...
import com.wdeath.wdlang.script.exceptions.PatternMatchingException;
...
public final class MatchExpression extends InterruptableNode implements Expression, Statement {
    ...
    public MatchExpression(ScriptProgram scriptProgram, Expression expression, List<Pattern> patterns) {
    ...
    public Value eval() {
        ...
        for (Pattern p : patterns) {
            ...
            if (p instanceof VariablePattern) {
                ...
                if (scriptProgram.getVariables().isExists(pattern.variable)) {
                    if (match(value, scriptProgram.getVariables().get(pattern.variable)) && optMatches(p)) {
                        return evalResult(p.result);
                    }
                } else {
                    scriptProgram.getVariables().define(pattern.variable, value);
                    if (optMatches(p)) {
                        final Value result = evalResult(p.result);
                        scriptProgram.getVariables().remove(pattern.variable);
                        return result;
                    }
                    scriptProgram.getVariables().remove(pattern.variable);
                }
            ...
            if ((value.type() == Types.ARRAY) && (p instanceof ListPattern)) {
                ...
                if (matchListPattern((ArrayValue) value, pattern)) {
                    ...
                    for (String var : pattern.parts) {
                        scriptProgram.getVariables().remove(var);
                    }
                    return result;
                }
            }
    ...
    private boolean matchListPattern(ArrayValue array, ListPattern p) {
        ...
        switch (partsSize) {
            ...
            case 1: // match arr { case [x]: x = arr ... }
                ...
                scriptProgram.getVariables().define(variable, array);
                if (optMatches(p)) {
                    return true;
                }
    ...
    private boolean matchListPatternEqualsSize(ListPattern p, List<String> parts, int partsSize, ArrayValue array) {
        ...
        for (int i = 0; i < partsSize; i++) {
            scriptProgram.getVariables().define(parts.get(i), array.get(i));
        }
        if (optMatches(p)) {
    ...
    private boolean matchListPatternWithTail(ListPattern p, List<String> parts, int partsSize, ArrayValue array, int arraySize) {
        ...
        scriptProgram.getVariables().define(parts.get(lastPart), tail);
